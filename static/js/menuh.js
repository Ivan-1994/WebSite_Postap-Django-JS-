...
function menuh12()
{
	var menuh12='<div class="menu1" align="left">';
...
function serchclan() {
	var id_clan = (1057, 1058, 1007, 1010, 1001, 1021, 1020, 1004, 1000, 1002, 1016, 1045, 1043,
               1034, 1006, 1033, 1030, 1039, 1052)
    var name_clan = ('Black Hearts', 'Black WOLFS', 'Dark Jokers', 'Equilibrium', 'Invisibles',
	...
	var c;
	document.getElementById('sostavSselect').innerHTML = '<option value=\'-1\'> - Выберите клан - </option>';
	for(var i=1;i<100;i++){
		if(i<10){
			c = '0' + i.toString();
		}else {c=i.toString();}
		$.ajax({ url: "http://api.unit-online.ru/clan?id=10"+c, dataType: "jsonp"  }).done(function (clan) {
			if(clan.status != 'fail'){
				console.log(clan.id);
				if (clan.id.toString().substring(2,3) == '0' )
					document.getElementById('sostavSselect').innerHTML += '<option value=\'' + clan.id.toString().substring(3,4) + '\' >' + clan.name + '</option>';
				else
					document.getElementById('sostavSselect').innerHTML += '<option value=\'' + clan.id.toString().substring(2,4) + '\' >' + clan.name + '</option>';
			}
		});
	}
}
...
function clanzone() {
	...
	"Деревня Большие Комары","Стадион","Вертолетная площадка","Мотель"];
		for(var i=0;i<24;i++)
		{
			...
			document.getElementById('zones').innerHTML += "<tr><td align='center'> <img src=\"http://unit-online.ru/icons/icon/clan"+result.zones[j].clan + ".png\">  </td><td align='center'>"+result.zones[j].name+"</td><td align='center'> Захвачена: "+result.zones[j].captured.substr(0,4)+"."+result.zones[j].captured.substr(5,2)+"."+result.zones[j].captured.substr(8,2)+"</td></tr>";
			}
		}
		});
}
...
function sostavScalc() {
			...
			if(c<10)
			{
				$.ajax({ url: "http://api.unit-online.ru/clan?id=100"+c, dataType: "jsonp"  }).done(function (result){
					$("#sostavS").html("<tr><td style='padding-bottom: 10px;'>Статус:</td><td style='padding-bottom: 10px;'>Персонаж:</td><td style='padding-bottom: 10px;'>Должность:</td></tr>");
						...
						for(var j=0;;j++){
							if(result.users[j]!=undefined){
								if(result.users[j].position==arr[i]) {
									...
									a = $("#sostavS").html();
									if (result.users[j].online === true){
										$("#sostavS").html(a + "<tr><td><span style='color: green;'> Online</span></td><td><img src=" + result.icon + "> " + result.users[j].name + " [" + result.users[j].level + "]" + "<a href=\"http://unit-online.ru/character?id=" + result.users[j].id + "\" target=\"_blank\"> <img src=http://forum.unit-online.ru/static/img/info.gif></a></td>    <td>" + result.users[j].position + "</td></tr>");
									}else if (result.users[j].online === false){
										$("#sostavS").html(a + "<tr><td><span style='color: #a41515;'> Offline</span></td><td><img src=" + result.icon + "> " + result.users[j].name + " [" + result.users[j].level + "]" + "<a href=\"http://unit-online.ru/character?id=" + result.users[j].id + "\" target=\"_blank\"> <img src=http://forum.unit-online.ru/static/img/info.gif></a>    <td>" + result.users[j].position + "</td></tr>");
									}
								}}else break;
						}}});
				}
...
function newonline() {
	$.ajax({ url: "http://api.unit-online.ru/online", dataType: "jsonp"  }).done(function (result) {
        ...
        for (var i=0;i<result.users.length;i++){
            if (!result.users[i].clan){
                document.getElementById('online-no-clan').innerHTML += result.users[i].name+" ["+result.users[i].level+"]"+"<a href=\"http://unit-online.ru/character?id="+result.users[i].id+"\"target=\"_blank\"> <img src=http://forum.unit-online.ru/static/img/info.gif></a><br>";
            }
        }
	});
}
function minu_o() {
	$.ajax({ url: "http://api.unit-online.ru/expendables?type=мины", dataType: "jsonp"  }).done(function (result) {
		for (var i=0;i<result.items.length;i++){

			$.ajax({ url: "http://api.unit-online.ru/exp?id=" + result.items[i].id.toString(), dataType: "jsonp"  }).done(function (result) {
				var c = '';
				c += '<div style="position:relative; float: left; height: 180px; width: 140px;">';
				c += '<img src="' + result.item.image + '"><br>';
				c += result.item.name + '<br>';
				c += result.item.type + '<br>';
				for (var j=0;j < result.item.params.length; j++){
					c += result.item.params[j].name + ' ' + result.item.params[j].value + "<br>";
				}
				document.getElementById('minu').innerHTML += c + '</div>';
...
function lekarstva_o() {
	$.ajax({ url: "http://api.unit-online.ru/expendables?type=лекарства", dataType: "jsonp"  }).done(function (result) {
		for (var i=0;i<result.items.length;i++){

			$.ajax({ url: "http://api.unit-online.ru/exp?id=" + result.items[i].id.toString(), dataType: "jsonp"  }).done(function (result) {
				var c = '';
				c += '<div style="position:relative; float: left; min-height: 135px; min-width: 350px;">';
				c += '<img src="' + result.item.image + '"><br>';
				c += result.item.name + '<br>';
				c += result.item.type + '<br>';
				for (var j=0;j < result.item.params.length; j++){
				...
				for (var i=0;i<result.item.effects.length; i++){
					if (result.item.effects[i].info != undefined)
						c += result.item.effects[i].info + ' ';
					if (result.item.effects[i].type != undefined)
						c += result.item.effects[i].type + ' ';
					if (result.item.effects[i].probability != undefined)
						c += result.item.effects[i].probability + ' ';
					if (result.item.effects[i].duration != undefined)
						c += result.item.effects[i].duration + ' ';
					c += "<br>"
				}
				document.getElementById('lekarstva').innerHTML += c + '<br></div>';
...
function patronu_o() {
	$.ajax({ url: "http://api.unit-online.ru/expendables?type=патроны", dataType: "jsonp"  }).done(function (result) {
		for (var i=0;i<result.items.length;i++){

			$.ajax({ url: "http://api.unit-online.ru/exp?id=" + result.items[i].id.toString(), dataType: "jsonp"  }).done(function (result) {
				var c = '';
				c += '<div style="position:relative; float: left; height: 180px; width: 150px;">';
				c += '<img src="' + result.item.image + '"><br>';
				c += result.item.name + '<br>';
				c += result.item.type + '<br>';
				for (var j=0;j < result.item.params.length; j++){
				...
				document.getElementById('patronu').innerHTML += c + '</div>';
...
function yeda_o() {
	$.ajax({ url: "http://api.unit-online.ru/expendables?type=еда", dataType: "jsonp"  }).done(function (result) {
		for (var i=0;i<result.items.length;i++){
			...
			$.ajax({ url: "http://api.unit-online.ru/exp?id=" + result.items[i].id.toString(), dataType: "jsonp"  }).done(function (result) {
				...
				document.getElementById('yeda').innerHTML += c + '</div>';
...
function granatu_o() {
	$.ajax({ url: "http://api.unit-online.ru/expendables?type=гранаты", dataType: "jsonp"  }).done(function (result) {
		for (var i=0;i<result.items.length;i++){

			$.ajax({ url: "http://api.unit-online.ru/exp?id=" + result.items[i].id.toString(), dataType: "jsonp"  }).done(function (result) {
				var c = '';
				c += '<div style="position:relative; float: left; height: 180px; width: 120px;">';
				c += '<img src="' + result.item.image + '"><br>';
				c += result.item.name + '<br>';
				c += result.item.type + '<br>';
				for (var j=0;j < result.item.params.length; j++){
				...
				document.getElementById('granatu').innerHTML += c + '</div>';
